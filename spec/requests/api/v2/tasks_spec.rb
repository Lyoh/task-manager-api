...
RSpec.describe 'Task API' do
  ...
  describe 'GET /tasks' do
    ...
    context 'When filter and sorting params are sent' do
      ...
      it 'returns only the tasks matching' do
        ...
        expect(returned_task_titles).to eq([notebook_task_1.title, notebook_task_2.title].sort)
      end
  ...
  describe 'PUT /tasks/:id' do
    ...
    context 'When the params are valid' do
      let(:task_params) {{ title: 'New task title test!' }}

      it 'returns status code 200' do 
        expect(response).to have_http_status(200)
      end

      it 'Returns json data for update task' do
        expect(json_body[:data][:attributes][:title]).to eq(task_params[:title])
      end
